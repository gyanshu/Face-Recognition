...
void  create_histogram(int * histogram, int ** image, int num_rows, int num_cols) {
    ...
    #pragma omp parallel for collapse(2) private(row), firstprivate(count,decimal)
...
int find_closest(int training_set[NUMOFPEOPLE][NUMOFFILE][256], int num_persons, int num_training, int size, int *test_image){
    ...
    int min = 1000;
    int temp[256];
    //these loops contains mostly the critical section therefore parallelism wouldn't be beneficial
